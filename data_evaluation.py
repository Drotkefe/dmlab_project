...
def load_data_into_df(db_name: str) -> pd.DataFrame:
    try:
        ...
        df["Year"] = df.apply(
            lambda x: datetime.now().year if x["Year"] == "new" else x["Year"], axis=1
        )
...
def linear_regression_modell_for_prediction_prices(
    train_data: pd.DataFrame, y: pd.DataFrame
):
    # print(df.corr())
    X = pd.get_dummies(train_data, columns=["Brand"], drop_first=True)
...
if __name__ == "__main__":
    ...
    linear_regression_modell_for_prediction_prices(df_for_LR, df["Price"])
